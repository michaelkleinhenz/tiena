 ...
 * Examples:
 * 
 * SimpleQueue<char> queue(10); // Max 10 chars in this queue
 * queue.push('H');
 * queue.push('e');
 * queue.count(); // 2
 * queue.push('l');
...
template<class T>
inline int SimpleQueue<T>::count() 
{
  return _count;
}

template<class T>
inline int SimpleQueue<T>::front() 
{
  return _front;
}

template<class T>
inline int SimpleQueue<T>::back() 
{
  return _back;
}
